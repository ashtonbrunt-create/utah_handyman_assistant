...
TASK: Write a professional change order document for a job already in progress.
A change order documents work added or changed after the original job was agreed, and the additional time it takes.
Structure (ALL CAPS headings): Original Job, Requested Change, Additional Work, Additional Time And Cost, Authorization.
Rules:
- Original Job briefly restates what was originally agreed.
- Requested Change describes exactly what the customer asked to add or change.
- Additional Work lists the specific new tasks as numbered lines.
- Additional Time And Cost states added hours at ${COMPANY.rate}. If materials are needed, note receipts provided at cost with no markup. Never invent hours not provided — use [CONFIRM: additional hours] if missing.
- Authorization ends with a clear sentence stating the customer approves the additional work and cost before it begins, with a line for their okay.
- Keep it under 250 words. Plain, clear language.
- No markdown symbols, no dividers, no bullets. Numbered lines for tasks.`,
